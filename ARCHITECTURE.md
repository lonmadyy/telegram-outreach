...
**Повторное добавление отключённого аккаунта.** `/remove_account` делает мягкое удаление (статус `disabled`, строка остаётся). Тот же телефон можно добавить заново через `/add_account`: проверка «уже добавлен» пропускает `disabled` (хелпер `is_reactivatable`), и на финале вместо создания новой строки вызывается `reactivate_account` — обновляется ТА ЖЕ запись (тот же `id` → история кампаний, дедуп `processed_clients` и FK сохраняются). По текущему решению реактивация поднимает аккаунт сразу в `active` (без повторного warmup), обнуляя счётчики и снимая остаточные ограничения. Событие лога — `account_reactivated`. Живые статусы (`active`/`pause`/`spam_blocked`/`warmup`) и `dead` реактивации не подлежат — для них `/add_account` по-прежнему отвечает «уже добавлен».
