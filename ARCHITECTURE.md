...
- **Мониторинг через @SpamBot.** Каждые 240 секунд (4 минуты) каждый аккаунт отправляет `/start` в `@SpamBot`, парсит ответ, обновляет свой статус. Ответ «no limits» снимает ограничения **спам-линии** (`spam_blocked`, сниженный лимит) — досрочно, не дожидаясь таймера карантина. Паузы `FloodWait` и тихих часов SpamBot **не отменяет**: у них собственные таймеры, по истечении которых аккаунт возвращает воркер (§6.3, §5.3).
...
**Граница ответственности SpamBot ↔ FloodWait.** `no_limits` снимает только ограничения
спам-линии: `spam_blocked` (PeerFlood-карантин, temporary-блок), `limit_reduced_until` и
остаточный `spam_unlock_at` вне пауз. Паузы со статусом `pause` и причиной `flood_wait` или
`quiet_hours` спам-чек **не трогает** (`is_spam_line_restricted`): FloodWait — локальный
rate-limit Telegram на действие, который ответ SpamBot не отменяет (досрочный ретрай лишь
возвращает ту же ошибку), а тихие часы — наше расписание. Такие паузы истекают по своему
таймеру, и аккаунт возвращает воркер (`is_pause_expired`, событие `account_reactivated`,
без уведомления). Уведомление «лимит восстановлен» шлётся только при реальном снятии
спам-ограничения.

**Кулдаун PeerFlood-карантина.** SpamBot не отражает PeerFlood-лимит (внутренний
антиспам-скоринг) и отвечает `no_limits`, пока Telegram ещё режет аккаунт — без защиты
12-часовой карантин снимался в ту же секунду «немедленным» чеком §6.4 (на проде: 116
PeerFlood за 72ч у одного аккаунта, средняя жизнь карантина 58с). Поэтому `no_limits`
НЕ снимает `spam_blocked`, если у аккаунта был `peer_flood` за последние
`peerflood_cooldown_hours` (настройка, по умолчанию 3ч; 0 = выкл). После кулдауна —
снимается как обычно (§7.4).
...
### 7.1. Расписание
APScheduler-задача `spamcheck_job` запускается **каждые 240 секунд** для **каждого активного или paused/spam_blocked аккаунта**. Для `disabled` и `dead` — пропускается. Если аккаунт перешёл в `dead`/`disabled` уже в рантайме (несколько путей: воркер при auth-ошибке, сам spam-чек), джоба на ближайшем тике видит нерабочий статус (`is_spamcheckable`), **снимает сама себя** (`remove_spamcheck_for_account`) и выходит, не контактируя со SpamBot — единый chokepoint вместо хуков в каждом dead-пути. В тихие часы (§5.3) плановый опрос пропускается (настройка `spamcheck_quiet_pause`, по умолчанию on): воркеры спят, проверять нечего, а ~360 пустых сообщений SpamBot'у за ночь иногда ловили собственный FloodWait. Ручной `/spamcheck` работает в любое время.
...
| `/set_proxy <phone\|id> <url\|none>` | Задать/сменить/снять прокси аккаунта (живой воркер перезапускается) |
...
| `/resume` | Возобновление приостановленной (paused) |
| `/stop` | Полная остановка кампании (→ cancelled) |
| `/reactivate <id>` | Вернуть отменённую (cancelled) кампанию в работу |
...
**Повторное добавление отключённого/мёртвого аккаунта.** `/remove_account` делает мягкое удаление (статус `disabled`, строка остаётся); сессия также может стать `dead` (auth-key отозван Telegram — нужна реавторизация). Тот же телефон можно добавить заново через `/add_account`: проверка «уже добавлен» пропускает `disabled` и `dead` (хелпер `is_reactivatable`), и на финале вместо создания новой строки вызывается `reactivate_account` — обновляется ТА ЖЕ запись (тот же `id` → история кампаний, дедуп `processed_clients` и FK сохраняются). По текущему решению реактивация поднимает аккаунт сразу в `active` (без повторного warmup), обнуляя счётчики и снимая остаточные ограничения. Событие лога — `account_reactivated` (`<прежний статус> → active`). Живые статусы (`active`/`pause`/`spam_blocked`/`warmup`) реактивации не подлежат — для них `/add_account` отвечает «уже добавлен».
...
```
[Главное меню]  (callback_data)
├── Аккаунты (menu:accounts) ──► список аккаунтов с статусами
├── Шаблоны (menu:templates) ──► список шаблонов (кнопка 👁 tpl:view:<id> — тело целиком) + создать
├── Новая кампания (menu:new_campaign) ──► FSM
├── Кампании (menu:campaigns) ──► список кампаний (то же, что /campaigns)
├── Статус (menu:status) ──► сводка кампаний + аккаунтов (то же, что /status)
└── Настройки (menu:settings) ──► список ключ-значение + редактирование
```

Инвариант: `menu:status` → сводка `/status`, `menu:campaigns` → список кампаний. Callback
и одноимённая reply-кнопка ведут в один и тот же хендлер (не путать «Статус» и «Кампании»).
